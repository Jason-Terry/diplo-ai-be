...
async def test_first_call_unparseable_second_call_recovers(agent, monkeypatch):
    ...
    blob, _, _usage = await agent._stream_and_parse("dummy prompt", stream_callback=None, channel="orders")
...
async def test_both_calls_unparseable_returns_empty_blob(agent, monkeypatch):
    ...
    blob, _, _usage = await agent._stream_and_parse("dummy", stream_callback=None, channel="orders")
    assert blob == {}
